    ...
    items.forEach(data => {
        ...
        list.innerHTML += `<div style="background:#fff;padding:12px;border:2px solid #e2e8f0;margin-bottom:8px;border-radius:12px;">
            ...
            <div style="display:flex;gap:6px;margin-top:8px;">
                <button style="background:#4f46e5;box-shadow:0 4px 0 #3730a3;padding:6px 12px;font-size:13px;width:auto;margin:0;border-radius:10px;" onclick="openEditModal('${data._id}','${data._col}','${dataStr}')">✏️ திருத்து</button>
    ...
    query.get().then((snapshot) => {
        ...
        rows.forEach((data, i) => {
            ...
            tbody.innerHTML += `<tr>
                ...
                <td><button class="danger-btn" style="margin:0!important;padding:5px 8px!important;font-size:12px!important;" onclick="openDeleteModal('${data._id}','quiz_scores','score')">🗑️</button></td>
            </tr>`;
        });
...
// ── Edit Question Modal ───────────────────────────────────────────
function openEditModal(docId, col, dataStr) {
    let data = typeof dataStr === 'string' ? JSON.parse(dataStr.replace(/&#39;/g,"'")) : dataStr;
    ...
    if(col === 'quiz_questions' && data.options) {
        optsSection.style.display = '';
        ['edit-opt1','edit-opt2','edit-opt3','edit-opt4'].forEach((id,i) => {
            let el = document.getElementById(id); if(el) el.value = (data.options||[])[i]||'';
        });
    } else { optsSection.style.display = 'none'; }
...
function printCertificate() {
    // Temporarily remove max-height/overflow from container for clean print
    const certScreen = document.getElementById('certificate-screen');
    const origMaxH = certScreen ? certScreen.style.maxHeight : '';
    const origOvf  = certScreen ? certScreen.style.overflow  : '';
    if (certScreen) {
        certScreen.style.maxHeight = 'none';
        certScreen.style.overflow  = 'visible';
    }
    // Small delay so browser reflows before print dialog
    setTimeout(() => {
        window.print();
        // Restore after print dialog closes
        setTimeout(() => {
            if (certScreen) {
                certScreen.style.maxHeight = origMaxH;
                certScreen.style.overflow  = origOvf;
            }
        }, 1000);
    }, 150);
}
