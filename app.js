...
window.mcqImageFile = null;
window.descImageFile = null;
...
// ==================== SETTINGS & FONT ====================
function toggleDarkMode() { const d = document.body.classList.toggle('dark-mode'); lsSet(K_DARK, d); const cb = document.getElementById('dark-mode-toggle'); if (cb) cb.checked = d; }
function applyDarkMode() { const isDark = lsGet(K_DARK, false); if (isDark) document.body.classList.add('dark-mode'); const cb = document.getElementById('dark-mode-toggle'); if (cb) cb.checked = isDark; }

function updateFontBtns() { const isLg = document.body.classList.contains('font-lg'); const nb = document.getElementById('font-btn-normal'); const lb = document.getElementById('font-btn-large'); if(nb) nb.classList.toggle('on', !isLg); if(lb) lb.classList.toggle('on', isLg); }
function setFontPref(s) { document.body.classList.toggle('font-lg', s === 'large'); lsSet(K_FONT, s === 'large'); updateFontBtns(); }
function applyFontSize() { const isLg = lsGet(K_FONT, false); if (isLg) document.body.classList.add('font-lg'); updateFontBtns(); }
...
function doLogin() {
    ...
    if (t && t.pwd === pwd) {
        ...
        if(user === 'admin') document.getElementById('admin-danger-section').style.display = 'flex';
        else document.getElementById('admin-danger-section').style.display = 'none';
...
function doLogout() {
    window.currentTeacher = null;
    localStorage.removeItem('ua_last_teacher_user');
...
function startVoiceTyping(inputId, lang = 'ta-IN') {
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SpeechRecognition) {
        popupError("உங்கள் பிரவுசரில் வாய்ஸ் டைப்பிங் வசதி இல்லை. Chrome-ஐப் பயன்படுத்தவும்.");
        return;
    }
    
    const recognition = new SpeechRecognition();
    recognition.lang = lang; // 'ta-IN' for Tamil, 'en-US' for English
    recognition.interimResults = false;
    recognition.maxAlternatives = 1;
    
    const inputEl = document.getElementById(inputId);
    const micBtn = inputEl.nextElementSibling;
    if(micBtn && micBtn.classList.contains('mic-btn')) micBtn.classList.add('recording');
    
    const originalPlaceholder = inputEl.placeholder;
    inputEl.placeholder = "🎤 பேசுங்கள்... (Listening...)";
    ...
    recognition.onerror = (event) => {
        popupError("குரல் பதிவில் பிழை: " + event.error);
...
// ==================== IMAGE UPLOAD LOGIC 🖼️ ====================
function previewImage(input, previewId, type) {
    const file = input.files[0];
    if (file) {
        if(type === 'mcq') window.mcqImageFile = file;
        if(type === 'desc') window.descImageFile = file;
...
async function generateAIQuestions() {
    ...
    try {
        ...
        // Typeset MathJax in AI Preview
        if (window.MathJax) { MathJax.typesetPromise([document.getElementById('ai-questions-list')]); }
...
function handleExcelFile(input) {
    ...
    const reader=new FileReader(); reader.onload=(e)=>{
        try {
            ...
            document.getElementById('excel-preview-count').textContent=`✅ ${questions.length} questions found`; let html='<table style="width:100%;text-align:left;"><thead><tr><th>#</th><th>Question</th><th>Answer</th></tr></thead><tbody>'; questions.slice(0,10).forEach((q,i)=>{html+=`<tr><td>${i+1}</td><td>${escapeHtml(q.question)}</td><td style="color:var(--grn);">${escapeHtml(q.answer)}</td></tr>`;}); html+='</tbody></table>'; document.getElementById('excel-preview-table').innerHTML=html; prev.style.display='block';
            
            // Typeset MathJax in Preview
            if (window.MathJax) { MathJax.typesetPromise([document.getElementById('excel-preview-table')]); }
...
async function uploadSingleMcq() {
    ...
    statusEl.textContent = '⏳ சேமிக்கப்படுகிறது...'; statusEl.className = 'smsg'; statusEl.style.display='block';
    
    try {
        let imageUrl = null;
        if (window.mcqImageFile) {
            imageUrl = await uploadImageToStorage(window.mcqImageFile);
        }

        const qData = { question, option1: opt1, option2: opt2, option3: opt3, option4: opt4, answer, type: 'mcq', class: classVal, subject, topic, term, difficulty: 'medium', createdAt: new Date().toISOString(), imageUrl: imageUrl || null };
        const ref = await db.collection('questions').add(qData); 
        lsSaveQuestion(ref.id, qData); 
        
        statusEl.textContent = '✅ சேர்க்கப்பட்டது!'; statusEl.className = 'smsg ok'; 
        
        // Reset Inputs
        ['single-question','single-opt1','single-opt2','single-opt3','single-opt4','single-image-input'].forEach(id => document.getElementById(id).value = ''); 
        document.getElementById('single-image-preview').style.display = 'none';
        window.mcqImageFile = null;
        updateSingleAnswerSelect(); 
        
        setTimeout(() => { statusEl.style.display = 'none'; }, 3000); 
        loadManageQuestions(); 
    } catch (error) { statusEl.textContent = '❌ ' + error.message; statusEl.className = 'smsg er'; }
}

// 🌟 UPLOAD DESC WITH IMAGE 🌟
async function uploadDescQuestion() {
    ...
    try {
        let imageUrl = null;
        if (window.descImageFile) {
            imageUrl = await uploadImageToStorage(window.descImageFile);
        }

        const qData = { question, answer, type: 'desc', class: classVal, subject, topic, term, difficulty: 'medium', createdAt: new Date().toISOString(), imageUrl: imageUrl || null };
        const ref = await db.collection('questions').add(qData); 
        lsSaveQuestion(ref.id, qData); 
        ...
        // Reset Inputs
        document.getElementById('desc-question').value = ''; document.getElementById('desc-answer').value = ''; document.getElementById('desc-image-input').value = '';
        document.getElementById('desc-image-preview').style.display = 'none';
        window.descImageFile = null;
...
const _buildQ = (q, idx, type) => {
    ...
    // Show Image
    const imageHtml = q.imageUrl ? `<div style="margin-bottom:14px; text-align:center;"><img src="${escapeAttr(q.imageUrl)}" style="max-width:100%; max-height:220px; border-radius:12px; border:1px solid var(--border); object-fit:contain;"></div>` : '';
...
async function loadManageQuestions() {
    ...
    try {
        ...
        // 🌟 Typeset MathJax in Managed Cards 🌟
...
async function editQuestion(id, type) {
    try {
        ...
        const editImageHtml = q.imageUrl ? `<div style="margin-bottom:12px; text-align:center;"><img src="${escapeAttr(q.imageUrl)}" style="max-width:100%; max-height:150px; border-radius:12px; border:1px solid var(--border);"></div>` : '';
        ...
        <div style="background:var(--white);border-radius:24px;width:100%;max-width:400px;overflow:hidden;box-shadow:0 20px 50px rgba(0,0,0,0.3);border:1px solid var(--border);">
            ...
            <div style="padding:24px;max-height:75vh;overflow-y:auto;">
                ${editImageHtml}
                <label class="flbl">Question</label><textarea id="edit-question" class="ftxt" rows="3">${escapeHtml(q.question||'')}</textarea>${optionsHtml}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (u && teacherLogins[u]) {
        document.getElementById('l-user').value = u;
        document.getElementById('l-pwd').value = teacherLogins[u].pwd;
        doLogin();
    }
});
